...
const ATOM_PKG_DIR = path.join(__dirname, '../atom-ide-pkg');
const features = {};
...
const atomDirs = fs.readdirSync(ATOM_PKG_DIR)
  .filter(item => item.indexOf('.') === -1)
  .map(item => path.join(ATOM_PKG_DIR, item));

/**
 * Get the "package.json" of all the features.
 */
featureDirs.concat(atomDirs).forEach(dirname => {
...
export function activate() {
  ...
  localStorage.removeItem(
    nuclidePack.getCanDeferMainModuleRequireStorageKey(),
  );
